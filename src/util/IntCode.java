...
//            System.out.println("Current Index: " + currentIndex);
            ...
            for (int position = String.valueOf(getMemoryValue(currentIndex)).length() - 3;
                 position >= 0; position--) {
                parameterMode.add(Integer.parseInt(String.valueOf(
                        String.valueOf(getMemoryValue(currentIndex)).charAt(position))));
            }

            long firstValue = parameterMode.size() < 1 || parameterMode.get(0) == 0
                    ? getMemoryValue(getMemoryValue(currentIndex + 1))
                    : parameterMode.get(0) == 1
                            ? getMemoryValue(currentIndex + 1)
                            : getMemoryValue(getMemoryValue(currentIndex + 1) + relativeBase);
            long secondValue = parameterMode.size() < 2 || parameterMode.get(1) == 0
                    ? getMemoryValue(getMemoryValue(currentIndex + 2))
                    : parameterMode.get(1) == 1
                            ? getMemoryValue(currentIndex + 2)
                            : getMemoryValue(getMemoryValue(currentIndex + 2) + relativeBase);
...
//                        System.out.println("set " + thirdValue + " to 0");
                ...
                if (opCode == 3) { // Take Input
                    Integer head = input.poll();
                    memory.put(parameterMode.size() < 1 || parameterMode.get(0) == 0
                            ? getMemoryValue(currentIndex + 1) : getMemoryValue(currentIndex + 1) + relativeBase, (long) (head != null ? head : 0));
                    currentIndex += 2;
...
//                        System.out.println("zero jump to: " + (currentIndex + 3));
    ...
    public void addToInput(int toAdd) {
        input.offer(toAdd);
    }
